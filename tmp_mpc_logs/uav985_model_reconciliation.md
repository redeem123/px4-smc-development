# UAV985 Physical Model Reconciliation

This is the model contract shared by the UAV985 Gazebo model, rate MPC, and
model-based SMC. Values marked **identified** must be replaced with hardware
measurements before unrestricted real flight.

## CAD quantities

| Quantity | Value | Status |
|---|---:|---|
| All-up mass | `0.985 kg` | Inventor |
| Diagonal arm length | `0.230 m` | CAD |
| Rotor x/y offset | `0.162635 m` | derived as `0.230/sqrt(2)` |
| Total `Jxx` | `0.0135 kg m^2` | supplied UAV table |
| Total `Jyy` | `0.0118 kg m^2` | supplied UAV table |
| Total `Jzz` | `0.0170 kg m^2` | supplied UAV table |
| Rotor assembly mass | `0.022 kg` each | Inventor |
| Rotor local inertia | `6.998e-6, 2.2647e-5, 2.1617e-5 kg m^2` | Inventor |

The PX4 body axes must be checked against the Inventor export before hardware
flight. The controller uses `x=roll`, `y=pitch`, `z=yaw`; swapping CAD axes is a
model error, not a tuning adjustment.

## Gazebo link split

The four rotor links total `0.088 kg`, so the base link is `0.897 kg`. With the
rotors at `z=0.060 m`, the assembled center of mass is `z=0.0053604 m` relative
...
|---|---:|
| `Ixx` | `0.01085589834 kg m^2` |
| `Iyy` | `0.00909330234 kg m^2` |
| `Izz` | `0.01225830679 kg m^2` |

Those base values assemble to the CAD totals above. Putting the total CAD
inertia directly on the base link would double-count the rotor contributions.
The current contract assumes that the quoted `0.985 kg` CAD total already
contains the optical-flow and range-sensor hardware. Under that assumption,
...
At `0.985 kg`, the nominal hover speed is `468.5 rad/s`, normalized hover
command is `0.3747`, and the local thrust slope is `8.76 N` per normalized motor
...
effectiveness of `3.484/3.484/0.757 Nm/unit` at collective command `0.30` and
`4.946/4.946/1.075 Nm/unit` at `0.50`, or `0.864x` and `1.227x` the hover value.
...
1. Confirm the Inventor body-axis mapping and all-up inertia with the exact
   battery, landing gear, flight computer, and payload used in flight.
